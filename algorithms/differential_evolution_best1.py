"""
Differential Evolution Best 1 (DE/best/1/bin) implementation for the GymProblem.
...
def differential_evolution_best1(problem: GymProblem, budget: int = 1000, pop_size: int = 20,
                           F: float = 0.8, CR: float = 0.9, seed: int | None = None,
                           print_every: int = 0):
    """Run DE/best/1/bin and return best solution and history."""
...
def main():
    ...
    problem = GymProblem()
    best_x, best_f, f_hist, _, best_rewards = differential_evolution_best1(
        problem,
        budget=args.budget,
    ...
    plt.plot(f_hist)
    plt.title('DE (best/1) best fitness history')
