"""
Differential Evolution Rand-to-Best 2 (DE/rand-to-best/2/bin) implementation for the GymProblem.

Mutation strategy:
  v = xa + F*(best - xa) + F*(xb - xc) + F*(xd - xe)

API mirrors `differential_evolution_rand2.py`.
"""
...
def differential_evolution_rand_to_best2(problem: GymProblem, budget: int = 1000, pop_size: int = 20,
                           F: float = 0.8, CR: float = 0.9, seed: int | None = None,
                           print_every: int = 0):
    """Run DE/rand-to-best/2/bin and return best solution and history."""
...
def main():
    ...
    problem = GymProblem()
    best_x, best_f, f_hist, _, best_rewards = differential_evolution_rand_to_best2(
        problem,
        budget=args.budget,
    ...
    plt.plot(f_hist)
    plt.title('DE (rand-to-best/2) best fitness history')
