"""
Differential Evolution Current-to-Best 1 (DE/current-to-best/1/bin) implementation.

Mutation strategy:
  v = xi + F*(best - xi) + F*(xb - xc)

Matches style of other DE variants in this repo.
"""
...
def differential_evolution_current_to_best1(problem: GymProblem, budget: int = 1000, pop_size: int = 20,
                           F: float = 0.8, CR: float = 0.9, seed: int | None = None,
                           print_every: int = 0):
    """Run DE/current-to-best/1/bin and return best solution and history."""
    ...
    while evals < budget:
        gen += 1
        for i in range(pop_size):
            # pick two distinct indices b,c != i
            idxs = [idx for idx in range(pop_size) if idx != i]
            b, c = np.random.choice(idxs, size=2, replace=False)
            xb, xc = pop[b], pop[c]

            xi = pop[i]
            # mutation: current-to-best/1
            mutant = xi + F * (best_x - xi) + F * (xb - xc)
            # crossover (binomial)
            cross = np.random.rand(n) < CR
            jrand = np.random.randint(n)
            cross[jrand] = True
            trial = np.where(cross, mutant, xi)
...
def main():
    ...
    problem = GymProblem()
    best_x, best_f, f_hist, _, best_rewards = differential_evolution_current_to_best1(
        problem,
        budget=args.budget,
    ...
    plt.plot(f_hist)
    plt.title('DE (current-to-best/1) best fitness history')
