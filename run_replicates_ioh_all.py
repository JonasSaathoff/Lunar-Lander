    ...
    for preset in presets:
        ...
        for algo in algos:
            for i in range(args.reps):
                ...
                # wrap env creation to disable gym's passive env checker for this instance
                # No per-instance env wrapping needed for targeted runs.
                # add disable_env_checker to simulation params so envs created
                # inside `GymProblem.play_episode` use that flag
                try:
                    gym_problem.simulation_params['disable_env_checker'] = True
                except Exception:
                    pass
