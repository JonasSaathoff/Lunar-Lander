...
def main():
    problem = GymProblem()
    # differential_evolution returns a 5-tuple: (best_x, best_f, f_hist, sigma_hist_placeholder, best_rewards)
    # unpack accordingly. sigma_hist_placeholder is unused here (kept for compatibility).
    best_x, best_f, f_hist, sigma_hist_placeholder, best_rewards = differential_evolution(problem, budget=5000)
    print(f"Best fitness: {best_f}")
    # sigma_hist_placeholder is intentionally None/placeholder for DE; no sigma/accept_rate returned
    problem.show(best_x)
